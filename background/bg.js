...
(() => {
    ...
    "node_modules/@mozilla/readability/Readability.js"(exports2, module2) {
      ...
      Readability2.prototype = {
        ...
        **/
        _grabArticle: function(page) {
          ...
          while (true) {
            ...
            for (var s = 0, sl = siblings2.length; s < sl; s++) {
              var sibling = siblings2[s];
              var append4 = false;
        ...
        **/
         ...
         * @return void
         **/
        _cleanMatchedNodes: function(e, filter4) {
      ...
      CSSOM.CSSStyleRule.parse = function(ruleText) {
        ...
        for (var character; character = ruleText.charAt(i); i++) {
          switch (character) {
            ...
            case '"':
              j = i + 1;
              index2 = ruleText.indexOf('"', j) + 1;
              if (!index2) {
                throw '" is missing';
              }
              buffer += ruleText.slice(i, index2);
              i = index2 - 1;
              break;
            case "'":
              j = i + 1;
            ...
            switch (character) {
              ...
              case '"':
                if (state === "url") {
                  ...
                  this.href = cssText.slice(i + 1, index2);
                  i = index2;
                  state = "media";
                }
                break;
  ...
  function addClass(value) {
    ...
    for (let i = 0; i < numElements; i++) {
      ...
      if (!className) {
        ...
        for (let j = 0; j < classNames2.length; j++) {
          const appendClass = `${classNames2[j]} `;
  ...
  function filterBySelector(selector, elements, options) {
    ...
    if (selector.some(isTraversal)) {
      const root4 = (_a2 = options.root) !== null && _a2 !== void 0 ? _a2 : getDocumentRoot(elements[0]);
  ...
  function findElements(root4, sel, options, limit) {
    const query2 = _compileToken(sel, options, root4);
  ...
  function filterElements(elements, sel, options) {
    ...
    if (els.length === 0)
      return els;
    const query2 = _compileToken(sel, options);
  ...
  function getLoad(parse8, render3) {
    return function load2(content, options, isDocument2 = true) {
      ...
      function initialize(selector, context, root4 = initialRoot, opts) {
        ...
        const r = typeof root4 === "string" ? [parse8(root4, options2, false, null)] : "length" in root4 ? root4 : [root4];
  ...
  var OpenElementStack = class {
    ...
    constructor(document, treeAdapter, handler4) {
  ...
  function endTagInForeignContent(p, token) {
    ...
    for (let i = p.openElements.stackTop; i > 0; i--) {
      ...
      const tagName18 = p.treeAdapter.getTagName(element);
  ...
  function parseDocument(data2, options) {
    ...
    new Parser2(handler4, options).end(data2);
  ...
  async function getHtml({ query: query2, timerange }) {
    ...
    const content = new Parser3({
      ...
      onprocessinginstruction(name, data2) {
        if (name.toLowerCase() === "!doctype")
          document.doctype = data2.slice(name.length).trim();
      },
      // <tagName>
      onopentag(name, attributes2) {
        ...
        if (isHTML) {
          if (ownerSVGElement) {
            node = append3(node, document.createElementNS(SVG_NAMESPACE, name), active);
        ...
        if (create3)
          node = append3(node, document.createElement(name), false);
        let end2 = node[END];
        for (const name2 of keys(attributes2))
          attribute(node, end2, document.createAttribute(name2), attributes2[name2], active);
      },
      // #text, #comment
      oncomment(data2) {
        append3(node, document.createComment(data2), active);
      },
      ontext(text3) {
        append3(node, document.createTextNode(text3), active);
      },
      // </tagName>
      onclosetag() {
  ...
  var setInnerHtml = (node, html3) => {
    ...
    const { childNodes } = parseFromString(document, ignoreCase(node), html3);
  ...
  var prop2 = (name) => name.slice(5).replace(/-([a-z])/g, (_, $1) => $1.toUpperCase());
  var handler = {
    get(dataset, name) {
      if (name in dataset)
  ...
  var TreeWalker = class {
    ...
    nextNode() {
      const $2 = this[PRIVATE];
      this.currentNode = $2.i < $2.nodes.length ? $2.nodes[$2.i++] : null;
  ...
  var Document2 = class extends NonElementParentNode {
    ...
    cloneNode(deep = false) {
      ...
      if (deep) {
        const end2 = document[END];
        const { childNodes } = this;
        for (let { length } = childNodes, i = 0; i < length; i++)
          document.insertBefore(childNodes[i].cloneNode(true), end2);
        if (doctype)
  ...
  var parseHTML3 = (html3, globals = null) => new DOMParser().parseFromString(
    html3,
    "text/html",
    globals
  ).defaultView;
  ...
  var root3 = freeGlobal_default || freeSelf || Function("return this")();
  var root_default = root3;
  ...
  var getSystemLanguage = () => import_webextension_polyfill2.default.i18n.getUILanguage().split("-")[0];
  ...
  async function getUserConfig() {
    const config = await import_webextension_polyfill3.default.storage.sync.get(defaultConfig);
    return defaults_default(config, defaultConfig);
  }

  // src/content-scripts/api.ts
  var cleanText = (text3) => text3.trim().replace(/(\n){4,}/g, "\n\n\n").replace(/ {3,}/g, "  ").replace(/\t/g, "").replace(/\n+(\s*\n)*/g, "\n");
  async function getWebpageTitleAndText(url, html_str = "") {
    let html3 = html_str;
    if (!html3) {
      let response;
      try {
...
Make sure the URL is correct.`,
    ...
    const doc = parseHTML3(html3).document;
    ...
    let text3 = cleanText(parsed.textContent);
  ...
  // src/background/bg.ts
  var manifest_version = import_webextension_polyfill5.default.runtime.getManifest().manifest_version;
  import_webextension_polyfill5.default.runtime.onInstalled.addListener(async () => openChatGPTWebpage());
  function openChatGPTWebpage() {
    import_webextension_polyfill5.default.tabs.create({
      url: "https://chat.openai.com/chat"
    });
  }
  if (manifest_version == 2) {
    import_webextension_polyfill5.default.browserAction.onClicked.addListener(openChatGPTWebpage);
  } else {
    import_webextension_polyfill5.default.action.onClicked.addListener(openChatGPTWebpage);
  }
  import_webextension_polyfill5.default.commands.onCommand.addListener(async (command) => {
    if (command === "toggle-web-access") {
      import_webextension_polyfill5.default.tabs.query({ active: true, currentWindow: true }).then((tabs) => {
        const tab = tabs[0];
        if (tab.url && tab.id && tab.url.startsWith("https://chat.openai.com/")) {
          import_webextension_polyfill5.default.tabs.sendMessage(tab.id, "toggle-web-access");
        }
      });
    }
  });
  import_webextension_polyfill5.default.runtime.onMessage.addListener((request) => {
    if (request === "show_options") {
      import_webextension_polyfill5.default.runtime.openOptionsPage();
    }
  });
  import_webextension_polyfill5.default.runtime.onMessage.addListener((message) => {
